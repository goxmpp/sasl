...
type Scram struct {
	...
	client_nonce    string // Client's nonce
	server_nonce    string // Server's nonce concatenated to client's nonce
...
func (s *Scram) nonce() string {
	if len(s.server_nonce) == 0 {
		s.server_nonce = s.cnonce() + s.gen.GetNonce()
	}
